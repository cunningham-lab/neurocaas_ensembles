## Evaluate groundtruth marker point probability on a GPU machine. 
import click
import os 
import json
...
@click.command("make a dictionary containing the probability estimates at the groundtruth location") 
...
def main(video_name,groundtruth,partperm,labellist,ensemblesfolder):
    ...
    for e in ensembles:
        ...
        for mi,m in enumerate(e["models"]):
            diff = m.get_groundtruth_probability(groundtruth,video_name,range(1001),partperm = [1,3,0,2])
            e["modeldiffs"]["model{}".format(mi+1)] = diff
            
    joblib.dump(ensembles,os.path.join(scriptdir,"script_outputs","probability_data_{}".format(idstring)))
